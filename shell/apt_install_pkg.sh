...
if [ ! -f "$PKG_LIST" ]; then
  echo "pkglist file not found. Please create a file named 'pkglist' and list the packages to install."
  exit 1
fi
...
while IFS= read -r pkgname; do
  ...
  if dpkg-query -W -f='${Status}' "$pkgname" 2>/dev/null | grep -q "installed"; then
    ...
    echo "$pkgname (version: $version) is already installed. Skipping installation."
    continue
  fi
  ...
  echo "Package '$pkgname' installed successfully. Version: $installed_version"
done < "$PKG_LIST"
